...
export function viewFor(state: MatchState, seat: Seat): PlayerView {
  ...
  return {
    ...
    you: {
      ...
      discards: [...(state.discardedBySeat?.[seat] ?? [])],
    },
    players,
    ...
    exchangeCounts: state.exchangeCounts ? { ...state.exchangeCounts } : null,
    historyComplete: Boolean(state.completedTricks && state.discardedBySeat && state.exchangeCounts),
